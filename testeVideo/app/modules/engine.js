...
/*#__PURE__*/
function () {
  function LocstorService() {
    _classCallCheck(this, LocstorService);

    this.name = 'localStorage';
  }

  _createClass(LocstorService, [{
    key: "init",
    value: function init() {
      return true;
    }
  ...
  return LocstorService;
}();

var PersistanceFake =
/*#__PURE__*/
function () {
  _createClass(PersistanceFake, null, [{
    key: "Create",
  ...
  function PersistanceFake() {
    _classCallCheck(this, PersistanceFake);

    this.name = "PersistanceFake";
    this.db = {};
    console.log("%c* Persistance - ".concat(this.name, " * "), "font-weight:bold; color:purple");
  }

  _createClass(PersistanceFake, [{
    key: "init",
    value: function init() {
      return true;
    }
    ...
    key: "saveObject",
    value: function saveObject(key, value) {
      var objects = this.getObject(SCORM_API.SUSPEND_DATA) || {};
      objects[key] = value;
    ...
    key: "saveQuiz",
    value: function saveQuiz(key, value) {
      var quizes = this.loadObject(QUIZES) || {};
      quizes[key] = value;
      this.saveObject(QUIZES, quizes);
    }
  }, {
    key: "loadQuiz",
    value: function loadQuiz(key) {
      var quizes = this.loadObject(QUIZES) || {};
      return quizes[key];
    }
  }, {
    key: "getQuizCollection",
    value: function getQuizCollection() {
    ...
    key: "setLessonStatus",
    value: function setLessonStatus(status) {
      this.set(SCORM_API.LESSON_STATUS, status);
    }
  }]);

  return PersistanceFake;
}();

var PersistanceService =
/*#__PURE__*/
function () {
  _createClass(PersistanceService, null, [{
    key: "Create",
    value: function Create() {
  ...
  function PersistanceService(scorm, locstor) {
    _classCallCheck(this, PersistanceService);

    this.scorm = scorm;
    this.locstor = locstor;
    this.persistance = this.scorm.ready ? this.scorm : this.locstor; // console.log(`%c* Persistance - ${this.persistance.name} * `, "font-weight:bold; color:purple");
  } // inicializa scorm
  ...
  _createClass(PersistanceService, [{
    ...
    key: "saveObject",
    value: function saveObject(key, value) {
      var objects = this.persistance.getObject(SCORM_API.SUSPEND_DATA) || {};
      objects[key] = value;
...
/*#__PURE__*/
function () {
  function ScormAPIService() {
    _classCallCheck(this, ScormAPIService);
  ...
  _createClass(ScormAPIService, [{
    ...
    key: "set",
    value: function set(key, value) {
      var response = this.scorm.set(key, value);
      this.scorm.save();
      return response;
    }
  }, {
...
function () {
  ...
  _createClass(Collection, [{
    key: "iterator",
    value: function iterator(startAt) {
      var _this = this;
      ...
      return {
        next: function next() {
          var result = count < _this.list.length ? {
            value: _this.base.get(_this.list[count++]),
            done: false
          } : {
            done: true
          };
    ...
    key: "nextOf",
    value: function nextOf(key) {
      return this.get(this.indexOf(key) + 1);
    }
  }, {
    key: "prevOf",
    value: function prevOf(key) {
      return this.get(this.indexOf(key) - 1);
    }
  }, {
    key: "set",
    ...
    key: "getKeyIndex",
    value: function getKeyIndex(_key) {
...
var path = function path(pagepath) {
  return pathfix('app/pages/' + pagepath);
};
...
pipwerks.SCORM.data.get = function (parameter) {
  ...
  if (scorm.connection.isActive) {
    ...
    if (API) {
      ...
      errorCode = debug.getCode(); //GetValue returns an empty string on errors
...
/*#__PURE__*/
function () {
  function Screen(model, persistance, parent) {
    var _this = this;
    ...
    if (this.isCollection()) {
      ...
      this.pages = models.map(function (m) {
        return new Screen(m, persistance, _this);
      });
  ...
  _createClass(Screen, [{
    ...
    key: "shuffleThe",
    value: function shuffleThe(models) {
      var shuffledIMap = this.loadData(this.shuffleKey) || shuffle(this.makeIndexMap(models.length));
      this.saveData(this.shuffleKey, shuffledIMap);
      return mapModel(shuffledIMap, models);
    }
  }, {
    key: "makeIndexMap",
    value: function makeIndexMap(length) {
      var indexMap = [];
    ...
    key: "saveData",
    value: function saveData(key, value) {
      var data = this.persistance.loadObject(this.uid) || {};
      data[key] = value;
    ...
    key: "pickTheSampleOf",
    value: function pickTheSampleOf(sample, models) {
      return models.splice(0, sample);
    }
  }, {
    key: "needToShuffle",
    value: function needToShuffle() {
      var need = this.model.behaviors.hasOwnProperty("shuffle");
      return need;
    }
  }, {
    key: "needToPickASample",
    value: function needToPickASample() {
      var need = this.model.behaviors.hasOwnProperty("sample");
      return need;
    }
  }, {
    key: "hasBehaviors",
    value: function hasBehaviors() {
      var has = this.model.hasOwnProperty("behaviors");
      return has;
    }
  }, {
    key: "render",
    ...
    key: "selectPage",
    value: function selectPage(uid) {
      var _this2 = this;

      if (this.isSimpleScreen()) return null;
      var pageToReturn = null;
      var index_found = this.getIndex(uid);
      ...
      if (index_found > -1) {
        ...
        pageToReturn = this.pages.reduce(function (last, current, index) {
          if (last == null) {
    ...
    key: "pageToReturnCondition",
    value: function pageToReturnCondition(pageToReturn) {
    ...
    key: "getIndex",
    value: function getIndex(uid) {
      var index = -1;

      if (this.isCollection() && typeof uid == 'string') {
        index = this.pages.findIndex(function (p) {
          return p.model.uid == uid;
        });
      }
    ...
    key: "walkFix",
    value: function walkFix(direction) {
      if (direction < 0) {
    ...
    key: "indexIsOutLimits",
    value: function indexIsOutLimits(index) {
    ...
    key: "minLimit",
    value: function minLimit() {
      return 0;
    }
  }, {
    key: "maxLimit",
    value: function maxLimit() {
      return this.pages.length - 1;
    }
  }, {
    key: "indexClampLimit",
    value: function indexClampLimit() {
      if (this.index >= this.maxLimit()) this.index = this.maxLimit();
      if (this.index <= this.minLimit()) this.index = this.minLimit();
    }
  }, {
    key: "isCollection",
    ...
    key: "isSimpleScreen",
    value: function isSimpleScreen() {
      return !this.isCollection();
    }
  }, {
    key: "inject",
    value: function inject(property, value) {
    ...
    key: "enterView",
    value: function enterView(direction) {
      try {
    ...
    key: "exitView",
    value: function exitView(direction) {
      try {
...
var LOADED_SCREEN_LIMIT = 5;

var ViewController =
/*#__PURE__*/
function (_Base) {
  _inherits(ViewController, _Base);

  function ViewController(element, model, persistance) {
    var _this;

    _classCallCheck(this, ViewController);

    _this = _possibleConstructorReturn(this, _getPrototypeOf(ViewController).call(this, element));
    _this.persistance = persistance;
    _this.model = model;
    _this.current_uid = null;
    _this.screen = new Screen(model, persistance);
  ...
  _createClass(ViewController, [{
    ...
    key: "scormReturn",
    value: function scormReturn(localData) {
    ...
    key: "loadScreen",
    value: function loadScreen(screen) {
      var _this2 = this;
      ...
      return new Promise(function (res, rej) {
        ...
        screen.element.onload = function () {
          screen.inject('navigate', {
            goto: _this2.goto.bind(_this2),
            next: _this2.next.bind(_this2),
            prev: _this2.prev.bind(_this2)
          }).inject('scorm', _this2.persistance).inject('bridge', bridge).onready();
          res(screen);
        };

        _this2.appendChild(screen);
      });
    }
  }, {
    key: "goto",
    value: function goto(uid) {
      var _this3 = this;

      var next = null;
      var direction = true;
      var current = this.screen.getCurrentScreen();
      ...
      if (next && next.uid != current.uid) {
        ...
        return this.loadScreen(next).then(function () {
          current.exitView();
          next.enterView();

          _this3.updateSlidesStates(direction, current.element, next.element);

          _this3.persistance.set(SCORM_API.LESSON_LOCATION, next.model.uid);
    ...
    key: "updateSlidesStates",
    value: function updateSlidesStates(avanca, currentSlide, nextSlide) {
...
function Debuggy(view) {
  ...
  var openclose = createDiv('openclose');
  openclose.appendChild(createDiv('arrow-round'));
  var telaatual = createDiv('telaatual');
  var debug = createDiv('debug');
  var pages = createDiv('pages');
  var controls = createDiv('controls');
  var title = createDiv('title', 'Flamboyant Engine 2.0');
  ...
  function createDebugBtns(screen, i) {
    ...
    var btn = makeButton((screen.parent ? screen.parent.uid + '>' : '') + screen.uid, function () {
  ...
  function readAllScreens(screen) {
    screen.pages.forEach(function (screen, i) {
  ...
  controls.appendChild(makeButton('AVANÇAR', function () {
    return view.next();
  }));
  controls.appendChild(makeButton('VOLTAR', function () {
    return view.prev();
  }));
...
(function () {
  var persistir = config.hasOwnProperty('salvarDados') ? config.salvarDados : true;
  var api = persistir ? PersistanceService.Create() : new PersistanceFake();
  window.scorm = api;
