events.on('ready', function () {
  ...
  for(var i = 1; i <= pageAll ; i++ ){
    if( pageCurrent < i ){
      $('.hit'+ i ).addClass('inativo');
    }
    else if( pageCurrent == i ){
      $('.hit'+ i ).addClass('atual');
    }
    else{
      $('.hit'+ i ).addClass('visitado');
    }
  }
  ...
  $('body').on('mousemove', function() {
    ...
    el.style.setProperty('--xAngle', -x*5+'deg');
    el.style.setProperty('--yAngle', y*5+'deg');
